...
if __name__ == '__main__':
    ...
    func_para =np.array(func(number=number,eff=0.9,price=pd_price))
    ...
    res = scipy.optimize.linprog(func_para,A_ub=bounds_au,b_ub=bounds_bu,bounds=bounds)

    for i in range(len(res.x)):
        x_dis =[]
        x_ch = []
        if i%2 !=0:
            x_dis.append(res.x[i])
        else:
            x_ch.append(res.x[i])
